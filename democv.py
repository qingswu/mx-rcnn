...
from rcnn.tester import cv_vis_all_detection
...
def demo_net(detector, im, video=False):
    """
    ...
    if video:
        cv_vis_all_detection(im_array, boxes_this_image, CLASSES, 0)
    else:
...
def parse_args():
    ...
    parser.add_argument('--video', dest='video', help='video input', type=str)
...
if __name__ == '__main__':
    ...
    if args.image:
        ...
        demo_net(detector, image)
        begin = time.time()
        demo_net(detector, image)
        end = time.time()
        print "Processing time:", "%.2f" %(end - begin), "ms"

    if args.video:
        vid = cv2.VideoCapture(args.video)
        # vid.set(1, 19000) # set starting frame instead of begining
        while vid.isOpened():
            ret, image = vid.read()
            if not ret:
                break
            # image = image[0:800, 0:1920] # cut unnecessary regions
            begin = time.time()
            demo_net(detector, image, True)
            end = time.time()
            print "Processing time:", "%.2f" %(end - begin), "ms"
